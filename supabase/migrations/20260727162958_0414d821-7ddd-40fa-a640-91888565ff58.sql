...
CREATE OR REPLACE FUNCTION public.core_resolve_missing_sku_pending_item(
...
BEGIN
  ...
  -- Idempotency: existing manual-missing-sku movement for the same sale
  ...
  -- Trigger trg_replacement_fund_balance only handles reclassification / adjustment
  -- types (verified). For sale_generated we must update the fund balance here,
  -- exactly once.
...
GRANT EXECUTE ON FUNCTION public.core_resolve_missing_sku_pending_item(uuid, numeric, text, boolean)
  TO authenticated, service_role;


CREATE OR REPLACE FUNCTION public.core_close_missing_sku_pending_item(
  p_pending_item_id uuid,
  p_replacement_event_id uuid
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  UPDATE public.core_fabrication_fund_pending_items
     SET status = 'resolved',
         ...
         last_action_at = now(),
         last_action_by = v_uid,
         notes = COALESCE(notes,'') || E'\n[missing_sku:replacement_event=' || COALESCE(p_replacement_event_id::text,'null') || ']'
   WHERE id = p_pending_item_id;
...
GRANT EXECUTE ON FUNCTION public.core_close_missing_sku_pending_item(uuid, uuid)
  TO authenticated, service_role;
