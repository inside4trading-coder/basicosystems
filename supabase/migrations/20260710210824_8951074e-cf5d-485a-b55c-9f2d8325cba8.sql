...
BEGIN
  ...
  -- Step 2: lock event
  SELECT * INTO v_event
  FROM public.core_replenishment_policy_events
  WHERE id = p_event_id
  FOR UPDATE;
  ...
  -- Step 3: idempotency BEFORE state validation
  ...
  -- Step 5: read current policy. Current policy is source of truth;
  -- event columns are only a historical fallback for legacy events.
  IF v_event.policy_id IS NOT NULL THEN
    SELECT * INTO v_policy
    FROM public.core_replenishment_policies
    WHERE id = v_event.policy_id;
  END IF;

  IF v_policy.id IS NULL AND v_event.core_product_id IS NOT NULL THEN
    SELECT * INTO v_policy
    FROM public.core_replenishment_policies
    WHERE core_product_id = v_event.core_product_id
  ...
  IF v_policy.id IS NULL AND v_event.woo_product_id IS NOT NULL THEN
    SELECT * INTO v_policy
    FROM public.core_replenishment_policies
    WHERE woo_product_id = v_event.woo_product_id
    ORDER BY updated_at DESC
    LIMIT 1;
  END IF;
  ...
  -- Step 6: resolve replacement product identity from current policy first;
  -- event snapshot remains fallback for old events without a policy row.
  ...
  IF v_replacement_product_id IS NULL AND v_replacement_woo_product_id IS NOT NULL THEN
    SELECT cp.id INTO v_replacement_product_id
    FROM public.core_products cp
    WHERE cp.woo_product_id = v_replacement_woo_product_id
    LIMIT 1;
  END IF;
  ...
  -- Step 7: cycle detection using recursive CTE (max depth 20)
  ...
  -- Step 9: canonicalize allocations, dedupe, validate ownership
  ...
  LOOP
    ...
    ELSIF v_woo_variation_id IS NOT NULL AND v_core_variant_id IS NULL THEN
      ...
      FROM public.core_product_variants cpv
      ...
      IF v_core_variant_id IS NULL THEN
        ...
        FROM public.core_woo_variant_map wvm
        WHERE wvm.woo_variation_id = v_woo_variation_id
        LIMIT 1;
      END IF;
    END IF;

    -- Ownership: variant must belong to replacement product
    ...
    -- Reject if points to original product/variant
  ...
  -- Step 10 + 11: route each allocation via central engine (preview)
  ...
  LOOP
    ...
    -- Resolve unit cost via central helper
    BEGIN
      SELECT unit_cost, cost_source
        INTO v_unit_cost, v_cost_source
      FROM public.resolve_core_operational_unit_cost(
        v_replacement_product_id,
        v_core_variant_id,
        v_replacement_woo_product_id,
        v_woo_variation_id
      ) LIMIT 1;
    ...
    -- Per destination requirements
    IF v_line_action = 'allow_internal_factory' THEN
      -- rename to internal_factory bucket
    ...
    ELSIF v_line_action = 'external_supplier_review' THEN
      -- accepts Woo identity, no core_product_id required
      NULL;
  ...
  LOOP
    ...
    IF v_line_action = 'internal_factory' THEN
      -- Upsert into core_production_needs (idempotent by replacement event + canonical key)
      SELECT id INTO v_need_id
      FROM public.core_production_needs
    ...
    ELSE
      -- external_supplier_review or manual_cost_review: delegate to central engine
      v_route := public.route_core_replenishment_candidate(
        p_source_type := 'replacement_policy_event',
        p_source_key := p_event_id::text || ':' || v_alloc->>'canonical_key',
        p_source_id := p_event_id,
