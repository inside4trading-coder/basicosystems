CREATE OR REPLACE FUNCTION public.core_apply_replacement_event(p_event_id uuid, p_allocations jsonb, p_confirmed_quantity numeric DEFAULT NULL::numeric, p_adjustment_reason text DEFAULT NULL::text, p_dry_run boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_route_arr) LOOP
    ...
    -- Bug A fix: reconocer 'allow_internal_factory' como internal_factory
  ...
  -- === Ejecutar conciliación financiera (idempotente, sin lógica ternaria) ===
  -- Bug B fix: COALESCE evita que NULL colapse el IF cuando falta financial_reconciliation.status
  IF v_do_reconcile
  ...
  FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_route_arr) LOOP
    ...
    -- Bug C fix: aceptar allow_internal_factory como internal_factory
