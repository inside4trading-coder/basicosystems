...
DROP FUNCTION IF EXISTS public.core_apply_replacement_event(uuid, jsonb, numeric, text, boolean);
...
BEGIN
  ...
  FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_alloc_arr) LOOP
    ...
    IF v_line_action = 'allow_internal_factory' THEN
      ...
      END IF;
    ELSIF v_line_action = 'external_supplier_review' THEN NULL;
    ELSIF v_line_action = 'manual_cost_review' THEN NULL;
    ELSE RETURN jsonb_build_object('error','unexpected_route_action','action',v_line_action); END IF;
...
-- Backfill: rellenar necesidades ya creadas por reemplazos que quedaron sin datos denormalizados
UPDATE public.core_production_needs AS n
SET sku = COALESCE(NULLIF(n.sku,''), sub.core_sku),
    product_name = COALESCE(NULLIF(n.product_name,''), sub.name),
    variant_sku = COALESCE(NULLIF(n.variant_sku,''), sub.variant_sku),
    variant_label = COALESCE(NULLIF(n.variant_label,''), sub.variant_label),
    size = COALESCE(NULLIF(n.size,''), sub.size),
    updated_at = now()
FROM (
  SELECT n2.id AS need_id,
         cp.core_sku,
         cp.name,
         cpv.variant_sku,
         cpv.variant_label,
         cpv.size
  FROM public.core_production_needs n2
  JOIN public.core_products cp ON cp.id = n2.core_product_id
  LEFT JOIN public.core_product_variants cpv ON cpv.id = n2.core_variant_id
  WHERE n2.source LIKE 'replacement_policy_event:%'
    AND (n2.product_name IS NULL OR n2.product_name = '' OR n2.sku IS NULL OR n2.sku = '')
) AS sub
WHERE n.id = sub.need_id;
