...
BEGIN
  ...
  IF v_variant_found THEN
    IF v_variant.cost_override_enabled = true AND v_variant.cost_structure_id IS NOT NULL THEN
  ...
  IF v_product_found THEN
    IF COALESCE(v_product.manual_unit_cost_usd, 0) > 0 THEN
    ...
    -- 6) product_unit_cost
    IF COALESCE(v_product.unit_cost, 0) > 0 THEN
