...
import { ArrowLeft, Search, RefreshCw, Plus, Link2, EyeOff, Ban, Download } from "lucide-react";
...
export default function CoreWooCandidates() {
  ...
  async function load() {
    ...
    const [{ data: c }, { data: p }] = await Promise.all([
      supabase.from("core_woo_product_candidates").select("*").order("created_at", { ascending: false }).limit(500),
  ...
  const filtered = useMemo(() => items.filter(c => {
    ...
    if (fSource !== "all" && c.detected_from !== fSource) return false;
    return true;
  }), [items, search, fStatus, fSource]);

  async function runSync(mode: "catalog" | "sales") {
    setSyncing(true);
    try {
      const { data, error } = await supabase.functions.invoke("core-woo-sync", { body: {}, method: "POST" });
      const params = new URLSearchParams({ mode });
      // supabase-js v2 invoke doesn't pass query params natively — use direct fetch
  ...
  async function createDraftFromCandidate(c: Candidate) {
    if (!c.woo_sku) return toast.error("Este candidato no tiene SKU. Asigna uno en WooCommerce antes de crear el Producto Core.");
    ...
    if (dup) return toast.error(`Ya existe un Producto Core con SKU ${c.woo_sku} (${dup.core_sku}). Usa "Asociar a Core existente".`);

    const { data: { user } } = await supabase.auth.getUser();
    const { data: newProd, error } = await supabase.from("core_products").insert({
      core_sku: c.woo_sku,
      name: c.woo_product_name || c.woo_sku,
      commercial_status: "draft",
      is_restockable: true,
      sku_source: "woocommerce",
      sync_status: "draft_from_woo",
      woo_product_id: c.woo_product_id || null,
      woo_product_name: c.woo_product_name,
      woo_sku: c.woo_sku,
      woo_status: c.woo_status,
      woo_stock_quantity: c.woo_stock_quantity,
      woo_regular_price: c.woo_regular_price,
      woo_sale_price: c.woo_sale_price,
    ...
    if (c.woo_variations && Array.isArray(c.woo_variations) && c.woo_variations.length > 0) {
      const variants = c.woo_variations.map((v: any, i: number) => {
        ...
        return {
          ...
          size: String(sizeAttr),
          variant_sku: v.sku || null,
          woo_variation_id: v.id || null,
          woo_sku: v.sku || null,
          woo_stock_quantity: v.stock_quantity ?? null,
          woo_regular_price: v.regular_price ?? null,
          woo_sale_price: v.sale_price ?? null,
          status: "active",
          sort_order: i,
        };
      });
    ...
    await supabase.from("core_woo_product_candidates").update({
      status: "creado_como_borrador",
      matched_core_product_id: newProd.id,
    }).eq("id", c.id);
  ...
  async function linkToExisting(c: Candidate, productId: string) {
    ...
    await supabase.from("core_woo_product_candidates").update({
      status: "mapeado", matched_core_product_id: productId,
    }).eq("id", c.id);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight">Productos WooCommerce pendientes</h1>
            <p className="text-sm text-muted-foreground">Productos detectados en WooCommerce (catálogo o ventas) que aún no existen en Productos Core.</p>
          </div>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">Sin candidatos. Ejecuta una sincronización.</TableCell></TableRow>
              ) : filtered.map(c => {
                ...
                return (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-right tabular-nums text-xs">
                      {c.woo_sale_price ? `${c.woo_sale_price}` : c.woo_regular_price ?? "—"}
                    </TableCell>
                    <TableCell className="text-right tabular-nums text-xs">{c.woo_stock_quantity ?? "—"}</TableCell>
                    <TableCell className="text-xs text-muted-foreground">
                      {new Date(c.created_at).toLocaleDateString()}
                      {c.source_order_id && <div>Orden #{c.source_order_id}</div>}
                    </TableCell>
                    <TableCell className="text-right">
                      {["pendiente", "conflicto", "requiere_sku"].includes(c.status) ? (
                        <div className="flex justify-end gap-1 flex-wrap">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => setCandidateStatus(c, "no_fabricable", "Marcado no fabricable")} title="No fabricable"><Ban className="h-3 w-3" /></Button>
                        </div>
                      ) : (
