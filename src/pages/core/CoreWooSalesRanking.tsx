              ...
              return (
                <Fragment key={g.key}>
                  <TableRow className={cn(STATUS_COLORS[g.coreStatus], "cursor-pointer")} onClick={() => toggleExpand(g.key)}>
                    ...
                    <TableCell className="text-xs text-muted-foreground">{g.lastAt ? new formatDMY(Date(g.lastAt)) : "—"}</TableCell>
                  ...
                  {isOpen && Array.from(g.variants.values()).sort((a, b) => b.units - a.units).map((v, i) => {
                    ...
                    return (
                      <TableRow key={g.key + "_v_" + i} className={rowCls}>
                        ...
                        <TableCell className="text-xs text-muted-foreground">{v.lastAt ? new formatDMY(Date(v.lastAt)) : "—"}</TableCell>
